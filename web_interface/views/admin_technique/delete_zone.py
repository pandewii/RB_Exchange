...
from core.models import ZoneMonetaire, Source # Import Source model
from users.models import CustomUser
from django_celery_beat.models import PeriodicTask # Import PeriodicTask
...
class DeleteZoneView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # --- FIX: Logique critique pour supprimer la tâche planifiée et la source avant la suppression en cascade de la zone ---
        log_details_tasks = ""
        source_obj = None
        if hasattr(zone, 'source') and zone.source:
            source_obj = zone.source
            if source_obj.periodic_task:
                task_name = source_obj.periodic_task.name
                task_pk = source_obj.periodic_task.pk
                source_obj.periodic_task.delete() # Delete the periodic task
                log_details_tasks = f" La tâche planifiée '{task_name}' (ID: {task_pk}) a également été supprimée."
        ...
        log_details += log_details_tasks

        # Capture info before deletion as source and zone objects will become invalid
        zone_name = zone.nom
        zone_pk_for_log = zone.pk
        ...
        log_action(
            ...
            target_user_id=None,
            level='info',
            zone_obj=None, # Pass None for zone_obj as it has been deleted
            source_obj=None
        )
